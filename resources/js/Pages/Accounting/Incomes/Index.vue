...
import TransactionToolbar from "@/Pages/Accounting/Transactions/TransactionToolbar.vue";
...
const filteredIncomes = computed(() => {
    ...
    return props.incomes.filter((item) => {
        ...
        const matchesCustomRange = (!from || txnDate >= from) && (!to || txnDate <= to);
    ...
    <AppLayout>
        <div class="p-6 space-y-6">
            ...
            <p class="text-sm text-gray-500">
                Total Records: {{ incomeCount }} | Total Income: Rs.
            ...
            <div class="bg-white shadow rounded-lg overflow-hidden">
                ...
                <table class="w-full text-sm text-left text-gray-600">
                    ...
                    <tbody>
                        <tr v-for="(income, index) in filteredIncomes" :key="income.id" class="text-center border-b">
                            <td class="px-4 py-3">{{ index + 1 }}</td>
                            <td class="px-4 py-3">{{ income.account?.sourceable?.name || "N/A" }}</td>
                            <td class="px-4 py-3">{{ income.receipt_no || "—" }}</td>
                            <td class="px-4 py-3">{{ income.account.date }}</td>
                            <td class="px-4 py-3">{{ income.account.description || "—" }}</td>
                            <td class="px-4 py-3 text-green-600 font-semibold">
                                {{ Number(income.account.amount).toLocaleString() }}
                            </td>
                        </tr>
                        <tr v-if="!filteredIncomes.length">
                            <td colspan="6" class="text-center text-gray-500 py-6">No income records found.</td>
                        </tr>
                    </tbody>
                    <tfoot class="bg-gray-50 font-semibold text-center text-sm">
                        <tr>
                            <td colspan="4" class="px-6 py-4 text-right">Total Income:</td>
